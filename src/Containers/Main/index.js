...
class Main extends Component {
  ...
  componentDidMount = async () => {
    ...
    let resultVideos = urls.map((url, index) => ({
      title: names[index],
      url,
      category: parents[index].name,
    }));
  ...
  applyLogic = () => {
    ...
    if (filterTextVideo !== "") {
      ...
      if (
        !(
          secondValue > principalVideo.duration ||
  ...
  show = () => {
    ...
    let resultList = this.props.resultList.filter((video) => {
      let title = video.title.toLocaleLowerCase();
      let filterText =
        typeof this.props.filterText === "string" &&
        this.props.filterText.toLowerCase().trim();
    ...
    let toShow = listaFiltrada.map((video, index) => {
      return (
        <div className="playlist__details__video" key={index}>
          <div
            className="playlist__details__video-content"
            onClick={() => this.onClickList(video)}
          >
            <video src={video.url} />
    ...
    return this.props.loadingFirebase ? (
      <img className="spinner" src={rolling} alt="spinner" />
    ) : (
      toShow
    );
  };
    ...
    return (
      <div className="principal__search">
        ...
        <section className="video-player">
          ...
          {this.props.principalVideo !== "" && (
            <FilterText execFunc={this.applyLogic} />
          )}
        </section>
        ...
        <section className="playlist">
          ...
          <div className="playlist__details">{this.show()}</div>
        </section>
      </div>
    );
  }
